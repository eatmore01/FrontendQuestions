...
- [Общее](#%D0%BE%D0%B1%D1%89%D0%B5%D0%B5)
  ...
  - [лонг пулинг long - polling ?](#%D0%BB%D0%BE%D0%BD%D0%B3-%D0%BF%D1%83%D0%BB%D0%B8%D0%BD%D0%B3-long---polling-)
- [сети](#%D1%81%D0%B5%D1%82%D0%B8)
  ...
  - [Что такое DNS](#%D1%87%D1%82%D0%BE-%D1%82%D0%B0%D0%BA%D0%BE%D0%B5-dns)
- [frontend](#frontend)
  ...
  - [critical renred path](#critical-renred-path)
- [React](#react)
...
- [ts js](#ts-js)
  ...
  - [debounc в js](#debounc-%D0%B2-js)
  - [Полиморфизм в js](#%D0%BF%D0%BE%D0%BB%D0%B8%D0%BC%D0%BE%D1%80%D1%84%D0%B8%D0%B7%D0%BC-%D0%B2-js)
...
- Ответ
  ...
  это способ передачи данных между клиентом и сервером, который происхдоить в несколько этапов

  - Клиент запрашивает страницу у сервера, используя обычный http

  - Запрошенная страница выполняет код, который запрашивает файл от сервера.
  ...
  Лонг пулинг используется когда клиент не может подключиться по веб сокету.
  Его особенность в том что при лонг пулинге нету постоянного потока запросов.
...
- Ответ
  ...
  это фундаментальные механизм которые отвечает за хранение и обработку доменных адресов, так же для соотношения айпи адрессов и доменов.
...
### critical renred path

- Ответ

  это те события в рендере, который должны обязательно произойти при рендере страницы.

  1. Построение html и css дерева.
  2. render three - объеденение дом и css дерева, для конечного предсталвения что будет отображаться в конце.
  3. layout
  4. painting
...
- Ответ
  ...
  [Доклад про компануд компонент с холидж](https://www.youtube.com/watch?v=4BByJUk5x7M)
...
- Ответ
  ...
  Отличие в том что интерфейс можно наследовать а тип нет, и то что если ты пишешь два одинаковых интерфейса они схлопнуться в один, а с типами будет ошибка о дубликации.

  intarface можно расширять с помощью ключевого слова extends
...
- Ответ
  ...
  позволяет привызвать контекст обычной функции.

  bind - привызвает контекст и возвращает функцию с эти контекстом

  call - сразу вызывает функцию с привязанным контекстом и туда передаются аргументы через запятую.

  apply - сразу вызывает функцию с привязанным контекстом но туда передются аргументы в виде массива.

  и call и apply возвращяют результат функции.

  как может выглядить:

  ```typescript
  function hello(greating: string) {
    return `${greating} ${this.name}`;
  }

  const person = { name: "bob" };

  const newBindGreatingsFunction = hello.bind(person, "hello");

  console.log(newGreatingsFunction()); //hello bob

  const resultCall = hello.call(person, "hello");

  console.log(resultCall); // hello bob

  const resultApply = hello.call(person, ["hello"]);

  console.log(resultApply); // hello bob
  ```
...
### Как удалить поле в объекте ?

- Ответ

  первый способ это с помощью delete, а второй способ с помощью диструктуризации

  ```typescript
  const obj = { name: "me", female: "male" };

  //первый способ мутировать объект, что не очень хорошо
  const deleteSuccess = delete obj.name; //вернет тру если удалилось мутировать
  console.log(newObj); //  { female: 'male' }

  //второй способ это диструктуризировать объект и создать новый без поля, что по идеи лучше
...
- Ответ
  ...
  это механизм который полезен допустим при вводе инпута.

  Допустим я ввожу в инпут какие то буквы, и только после того как я остановился начинается отсчет допустим 500мс, затем отправляется запрос на сервер.
  Этот механизм помогает оптимизировать приложение за счет менее частых запросов на сервер.
...
- Ответ
  ...
  полиморфизм - это способность функции обрабатывать значения разных типов

  в основном полиморфиз достигается с помощью ad-hoc методов, то есть

  - приведение типов
  - оберти

  под обертками я понимаю new String(), new Number() и тд.

  под приведением типов это as, и тд.
