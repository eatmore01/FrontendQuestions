...
- Отувет
  ...
  useEffect - тоже взаимодействие с жизненным циклом компонента, тоесть useEffect вызывается при монтировании компонента,
  и он выполняет какой то колбэк, при изменении зависимостей
  ...
  хуе useCallback - запоминает ссылку на функцию, как бы мемоизируя ее и перерендеривая ее только при изменении кортежа зависимостей

  useMemo - для созхраренняи значения между ререндерами
...
- Ответ
  ...
  прототипы это фундаментальная штука в js, и они лежат в основе каждой функции обхекта и тд и позволяет наследовать меторды разные, и есть там прототип function которая содержит метод там apply bind и тд, есть прототип объекта которй содержит метод toString().

  Вроде прото еще позволяет поменять прототип объекта на другой.
...
- Ответ
  ...
  чистая функция эта та функиця которая не имеет сайд эффектов(то есть не изменяет ничего вне себя, не делбает запросы,
...
- Ответ
  ...
  То есть это еще возморджность функции сохранять доступность к скоупу
...
- Ответ
  ...
  это обобщенный тип, если простыми словами как я понимаю, это возможность создать динамический который ты будешь затем определять и использовать в функции для типизации даты какой то.

  и позволяешь прокидывать динамические типы и от них уже по разному типизировать и работать с ними.

  еще можно сказать что это как функция толкьо для типов, мы можем туда передать параметр и с ним уже бубениться
