...
- [все вопросы](#%D0%B2%D1%81%D0%B5-%D0%B2%D0%BE%D0%BF%D1%80%D0%BE%D1%81%D1%8B) - [Принципе программирования?](#%D0%BF%D1%80%D0%B8%D0%BD%D1%86%D0%B8%D0%BF%D0%B5-%D0%BF%D1%80%D0%BE%D0%B3%D1%80%D0%B0%D0%BC%D0%BC%D0%B8%D1%80%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D1%8F)
  - [frontend](#frontend)
...
### Мемоизация, хуки, когда стоит использоват?
...
- Отувет
  ...
  useId - для уникального идентификатора
...
2.  удобство передавать ее как колбэк тоесть () => {}
3.  this контекст, стрелочные функции берут контекст на область видимости выше, а если объявлена
...
основные стобцы это callStack, web api и task quey(очеред задач)

в коллстеке хранятся все синхронные задачи

промисы таймаймуты и тд храняться веб апи и все задачи из них идут в task queue и они пораждают микро таски и макротаски

сначала синхронные задачи(в callstack) -> микротаски (все абсолютно только потом макро) -> макро таски (очередь задач, очередь событий)

прикол в том что в очереди задач, что первые задачи которые попали тудап первые выполняться( first in first out )

а в стеке первая задача которая вошла выйдет последняя
...
и затьем общаться с ней из вне.
...
- Ответ
  ...
  это обобщенный тип, если простыми словами и как я понимаю это как аргументы функции только, куда мы
  прокидываем не параметры а типы, которые затем будут использоваться в типизировании чего то внутри функции
