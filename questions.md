...
- Ответ
  ...
  SSG это статик сайт генерейтинг, это когда отдается на клиент пр билде статичный html
  этот статичный html лежит на промежуточных серверах cdn вроде называется
...
### Всегда ли стоит использовать uesMemo, memo и тд?

- Ответ

  нет, нужно использовать когда явно нужно, потому что это довольно дорогая операция,
